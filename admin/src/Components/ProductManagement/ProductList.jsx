...
const fetchInventory = async ({ queryKey }) => {
    ...
    const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/api/v1/inventory/list`, { params: { page, limit } });
    return data;
};
...
const ProductList = ({ setShowForm, showForm }) => {
    ...
    const handleSave = async (updatedProduct) => {
        try {
            const res = await axios.put(`${import.meta.env.VITE_API_URL}/api/v1/inventory/${updatedProduct._id}`, updatedProduct);
    ...
    const handleDelete = async (productId) => {
        ...
        if (confirmed) {
            try {
                await axios.delete(`${import.meta.env.VITE_API_URL}/api/v1/inventory/${productId}`);
                queryClient.invalidateQueries(['inventory']);
